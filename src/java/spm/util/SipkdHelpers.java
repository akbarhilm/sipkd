...
public class SipkdHelpers {
    ...
    public static boolean compareDateWithCurrent(final String tgl) {
        ...
        try {
            ...
            logger.debug("CEK INPUT DATE === " + inputDate);
            logger.debug("CEK currentTimeMillis === " + new java.util.Date(System.currentTimeMillis()));
            /* 
             if (inputDate.compareTo(new java.util.Date(System.currentTimeMillis())) > 0) {
             status = true;
             }*/
            if (DateUtils.isSameDay(inputDate, current)) {
                status = true;
            } else if (inputDate.after(current)) {
                status = true;
            }
