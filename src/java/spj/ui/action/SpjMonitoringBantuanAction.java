...
import spj.config.PropertiesAplikasi;
 ...
 */
@Controller
@RequestMapping("/monitoringbantuan")
public class SpjMonitoringBantuanAction {

    private static final Logger log = LoggerFactory.getLogger(SpjMonitoringBantuanAction.class);
    @Autowired
    CetakReportServices cetakReportServices;
    
    @Autowired
    CetakValidasiSPJServices spjService;
    ...
    @RequestMapping(value = "/indexmonitoringbantuan", method = RequestMethod.GET)
    public String index(HttpServletRequest request) {
        final Pengguna pengguna = (Pengguna) request.getSession().getAttribute("pengguna");
              
        return "monitoring/monitoringbantuan";

    }

    
    
  @RequestMapping(value = "/json/prosescetakbantuan", method = RequestMethod.GET)
   public void processRequest(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            map.put("np", spjService.getnilaiparam(map));
           
              
                        map.put("ID_SKPD",idskpd);
                        map.put("TAHUN",tahunAnggaran);
                         map.put("TANGGAL",tgll);
                        List<Map> listhasil = spjService.getnilaiparam(map);
                        ...
                        map.put("pathreport", pathReport + "/Report_MonitoringPengeluaran_Bantuan.jasper");
                        JasperPrint jasperPrint = cetakReportServices.cetakReport(map);
                        //JasperPrint jasperPrint = JasperFillManager.fillReport( pathReport+"/Report_MonitoringPengeluaran_Bantuan.jasper", map, jdbcConnection);
                        final String filename = tahunAnggaran+"-"+"SPJ-MONITORING-BANTUAN"+"-"+idskpd+".pdf";
