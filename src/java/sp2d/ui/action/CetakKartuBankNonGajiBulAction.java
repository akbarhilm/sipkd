 ...
 */
@Controller
@RequestMapping("/cetakkartubanknongajibul")
public class CetakKartuBankNonGajiBulAction {

    private static final Logger log = LoggerFactory.getLogger(CetakKartuBankNonGajiBulAction.class);
    @Autowired
    ...
    @RequestMapping(value = "/indexnongajibul", method = RequestMethod.GET)
    public String indexspmup(HttpServletRequest request) {
        ...
        return "cetak/kartubank/kartubanknongajibul";
   ...
   public void processRequest(HttpServletRequest request, HttpServletResponse response) {
        ...
        final String kodewilayah = request.getParameter("kproses");
            ...
            final String tgl = SipkdHelpers.getStringDateFormatFromString(tanggal, "yyyyMM", "MM/yyyy");
            final String saldoawal =  request.getParameter("sa");
            final String namwil = request.getParameter("namwil");
            log.debug("TGL  BELUM KONVERSI = "+tanggal);
            log.debug("TGL = "+tgl);
            log.debug("SALDO AWAL = "+saldoawal);
        try {
            ...
            map.put("np", cetakValidasiSP2DServices.getnilaiparam(map));
                        ...
                        JasperPrint jasperPrint = JasperFillManager.fillReport( pathReport+"/Report_KartuBank-KPKD_BLN.jasper", map, jdbcConnection);
                        final String filename = tahunAnggaran+"-"+"KARTUBANKNONGAJI-BULANAN"+"-"+kodewilayah+"-"+tanggal+".pdf";
