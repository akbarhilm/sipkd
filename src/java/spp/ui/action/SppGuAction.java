...
import spp.services.SppGuServices;
...
public class SppGuAction {
    ...
    public ModelAndView add(final SppGu sppGu, final HttpServletRequest request) {
        ...
        if (listSkpd.isEmpty() || (!listSkpd.isEmpty() && listSkpd.size() > 1)) {
            ...
            paramPagu.put("idspp", 0);
            //paramPagu.put("idspp", idspp);

            // final Map<String, String> dataPagu = sppGuServices.getDataPagu(paramPagu);
            final Map<String, BigDecimal> dataPagu;

            if (SipkdHelpers.getIntFromString(tahunAnggaran) > 2015) {
                dataPagu = sppGuServices.getDataPaguBKU(paramPagu);

            } else {
                dataPagu = sppGuServices.getDataPagu(paramPagu);
    ...
    public ModelAndView add(@PathVariable Integer idskpd, final SppGu sppGu, final HttpServletRequest request) {
        ...
        paramBank.put("tahun", SipkdHelpers.getIntFromString(tahunAnggaran));
        final Map<String, String> dataBank = sppGuServices.getBankRekByIdSkpd(paramBank);
        final Map<String, String> dataBankDki = sppGuServices.getBankDki(paramBank); // data bank nya diganti jadi BANK DKI
        //log.debug(" >>>>>>>>>>>>>  " + dataBank.toString());
        /*sppGu.setKodeBank(dataBankDki.get("C_BANK"));
        sppGu.setKodeBankTransfer(dataBankDki.get("C_BANK_TRANSFER"));
        sppGu.setNamaBankTransfer(dataBankDki.get("N_BANK_TRANSFER"));
        sppGu.setIdBank(dataBankDki.get("I_ID").toString());*/ // DATA BANK NGAMBIL KE TRSKPDBANKREK SEMUANYA

        sppGu.setNomorRekBank(dataBank.get("I_REK_BANKSPM"));
        
        sppGu.setKodeBank(dataBank.get("C_BANK"));
        sppGu.setKodeBankTransfer(dataBank.get("C_BANK_TRANSFER"));
        sppGu.setNamaBankTransfer(dataBank.get("N_BANK"));
        sppGu.setIdBank(dataBank.get("I_IDBANKPFK").toString());
        ...
        //final Map<String, BigDecimal> dataPagu = sppGuServices.getDataPagu(paramPagu);
        final Map<String, BigDecimal> dataPagu;
        ...
        log.debug("sisaUang ======== " + dataPagu.get("sisaUang"));
        if (dataPagu != null) {
            request.setAttribute("totalSpp", SipkdHelpers.formatBigDecimalPropertyEditor(dataPagu.get("TOTAL_SPP")));
            request.setAttribute("totalSpj", SipkdHelpers.formatBigDecimalPropertyEditor(dataPagu.get("TOTAL_SPJ")));
            request.setAttribute("nilaiSisaPaguSpp", SipkdHelpers.formatBigDecimalPropertyEditor(dataPagu.get("SISA_UANG")));
            request.setAttribute("presentase", dataPagu.get("PERSEN"));
    ...
    public String prosessimpanspdbtl(@Valid @ModelAttribute("refsppgu") SppGu sppGu,
        ...
        if (result.hasErrors()) {
            ...
            log.debug("================= simpan spp gu =================");
            sppGu.setNamaTujuan(namatujuan);
    ...
    public ModelAndView edit(@PathVariable Integer idspp, final HttpServletRequest request, SppGu sppGu,
        ...
        request.setAttribute("totalSpj", SipkdHelpers.formatBigDecimalPropertyEditor(dataPagu.get("TOTAL_SPJ")));
        request.setAttribute("nilaiSisaPaguSpp", SipkdHelpers.formatBigDecimalPropertyEditor(dataPagu.get("SISA_UANG")));
        request.setAttribute("validasiPaguSpp", SipkdHelpers.formatBigDecimalPropertyEditor(dataPagu.get("PAGU_SPP")));
    ...
    @RequestMapping(value = "/json/getStatusGU", method = RequestMethod.GET)
    public @ResponseBody
    Integer getStatusGU(final HttpServletRequest request) {
        final String tahunAnggaran = (String) request.getSession().getAttribute("tahunAnggaran");
        final String idskpd = request.getParameter("idskpd");
        ...
        return sppGuServices.getStatusGU(param);
    }

    @RequestMapping(value = "/json/getBanyakGU", method = RequestMethod.GET)
    public @ResponseBody
    Integer getBanyakGU(final HttpServletRequest request) {
        final String tahunAnggaran = (String) request.getSession().getAttribute("tahunAnggaran");
        final String idskpd = request.getParameter("idskpd");
        ...
        return sppGuServices.getBanyakGU(param);
    }
