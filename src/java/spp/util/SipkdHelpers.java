...
public class SipkdHelpers {
    ...
    public static String getStringDateFormatFromString(String param, String formatOut, String formatIn) {
        ...
        if (param != null && StringUtils.isNotEmpty(param) && StringUtils.isNotBlank(param)) {
            try {
                final SimpleDateFormat sdf = new SimpleDateFormat(formatIn, Locale.US);
                final SimpleDateFormat sdfout = new SimpleDateFormat(formatOut);
                //logger.debug("hasil date = " + sdf.parse(param));
                //logger.debug("format out = " + formatOut);
                return sdfout.format(sdf.parse(param));
            } catch (ParseException ex) {
                //final SimpleDateFormat sdf1 = new SimpleDateFormat(formatIn);
                //logger.debug("format1 in = " + sdf1.parse(param));
                //logger.debug("error = " + ex.getMessage());
                //logger.debug("format1 in = " + formatIn);
                //logger.debug("format1 out = " + formatOut);
                return null;
            }

        } else {
            //logger.debug("format2 out = " + formatOut);
            return null;
        }
