...
function gridspj() {
    ...
    if (typeof myTable == 'undefined') {
        myTable = $('#spjtable').dataTable({"bPaginate": true,
            ...
            "bFilter": false,
            "sAjaxSource": getbasepath() + "/koreksiakunbop/json/listspjkoreksi",
            "aaSorting": [[1, "asc"]],
...
function simpanKoreksi() {
    ...
    if (jumbarisspj > 0) {
        var varurl = getbasepath() + "/koreksiakunbop/json/prosessimpankoreksi";
        var dataac = [];
        var nilailist = [];
        var i;
        var banyak = 0;
        ...
        return   $.ajax({
            ...
            $('#btnSimpan').attr("disabled", false);
            //window.location.href = getbasepath() + "/koreksiakunbop/index"; // ke form index
