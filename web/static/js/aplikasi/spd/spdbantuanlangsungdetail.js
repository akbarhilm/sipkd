$(document).ready(function () {
    ...
    loadupdate(getbasepath() + "/spd/pengajuanbl/json/getlistspddetailbyskpddantahun");
    statusbreadcrum();
    $('#isexist').change(function () {
        var idskpd = $("#idskpd").val();
        var idSpd = $("#idSpd").val();
        var tahunAnggaran = $("#tahunAnggaran").val();
        gridspdbtllist(getbasepath() + "/spd/pengajuanbl/json/getlistspddetailbyskpddantahun", idskpd, idSpd, tahunAnggaran);
        if ($(this).is(":checked")) {
...
function getpagudanspd(id) {
    ...
    var noSpd = $("#noSpd").val();
    statusbreadcrum();
    $.getJSON(getbasepath() + "/spd/pengajuanbl/json/getanggarandanspdbantuanlangsung/" + id,
            function (result) {
    ...
    if (noSpd && $.trim(noSpd).length > 0) {
        ...
        gridspdbtllist(getbasepath() + "/spd/pengajuanbl/json/getlistspddetailbyskpddantahun", id, idSpd, tahunAnggaran);
    }
...
function simpan() {
    ...
    var idSpd = $("#idSpd").val();
    var tahunAnggaran = $("#tahunAnggaran").val();
    submispddetail(getbasepath() + '/spd/pengajuanbl/json/addspddetail', getbasepath() + "/spd/pengajuanbl/json/getlistspddetailbyskpddantahun", idskpd, idSpd, tahunAnggaran);
}
function pindahhalamanadepan() {
    if ($("#idskpd").val().trim()) {
        window.location.replace(getbasepath() + "/spd/pengajuanbl/bl/indexskpd/" + $("#idskpd").val().trim());
    } else {
        window.location.replace(getbasepath() + "/spd/pengajuanbl/bl/index");
    }
...
function gridspdbtllistbyidkegiatan(    ) {
    if (typeof detailTable == 'undefined') {
        detailTable = $('#btlspdbykegiatantable').dataTable({
            ...
            "bScrollCollapse": true,
            "bFilter": false,
            "sAjaxSource": getbasepath() + '/spd/pengajuanbl/json/getlistspddetailbyidkegiatandantahun',
            "aaSorting": [[1, "asc"]],
            "fnDrawCallback": function () {
                $(".inputmoney").priceFormat({prefix: "", suffix: "", centsSeparator: ",", thousandsSeparator: ".", limit: 15});
            },
