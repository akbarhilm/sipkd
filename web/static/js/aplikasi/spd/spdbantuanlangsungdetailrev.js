    ...
    if (typeof detailTable == 'undefined') {
        detailTable = $('#btlspdbykegiatantable').dataTable({
            ...
            "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull, oSettings) {
                ...
                var nilaiAnggaranTAPD = accounting.formatNumber(aData['nilaiAnggaranTAPD'], 2, '.', ",");
                $('td:eq(4)', nRow).html(nilaiAnggaranTAPD);
                var nilaiAnggaranSPDSisa = accounting.formatNumber(aData['nilaiAnggaranSPDSisa'], 2, '.', ",");
                $('td:eq(5)', nRow).html(nilaiAnggaranSPDSisa);
                ...
                var inputspd = "<input type='text' id='nilaiSpdX" + aData['idAkun'] + "'    name='nilaiSpdX" + aData['idAkun'] + "'   value='" + accounting.formatNumber(nilaiAnggaranSPDCurrent, 2, '.', ",") + "'   class='inputmoney' onkeyup=pasangvalidatebesarperfield(" + aData['idAkun'] + ")  onfocusout='validasinilaispp(" +  aData['idAkun'] + ")'  />";
          
                $('td:eq(6)', nRow).html(inputspd + sisaspd);
                ...
                /*
                 var nilaiSPDYangSudahDiserap = "<input type='hidden' id='nilaiSPDYangSudahDiserap' value="+aData['nilaiSPDYangSudahDiserap']+" />";
                 $('td:eq(7)', nRow).html(nilaiSPDYangSudahDiserap+accounting.formatNumber(aData['nilaiSPDYangSudahDiserap'], 2, '.', ","));
            ...
            "aoColumns": [
                ...
                {"mDataProp": "namaAkun", "bSortable": false, sDefaultContent: "-", "sWidth": "25%"},
...
function pasangvalidatebesarperfield(idakun) {
    ...
    var nilaiSpd = eval(accounting.unformat($("#nilaiSpdX" + idakun).val(), ",")) * 10;//parseFloat($("#nilaiSpdX" + idakun).autoNumeric('get'));
    var nilaiangg = $("#idSisaTAPDX" + idakun).val();//$("#idSisaTAPDX" + idakun).autoNumeric('get') ;
    var nilaiSPDYangSudahDiserap = $("#nilaiSPPcek" + idakun).val(); //$("#nilaiSPDYangSudahDiserap").val();
    var status = nilaiSpd >= nilaiangg && nilaiSpd > 0 ? false : true;
    ...
    //console.log("nilai SPD input = " + nilaiSpd);
    if (!status) {
        $("#nilaiSpdX" + idakun).prop('readonly', true);
        // /*
        console.log("Sisa SPD = " + nilaiangg);
        console.log("nilai SPP = " + nilaiSPDYangSudahDiserap);

        //*/
        bootbox.alert("Nilai SPD Tidak Boleh Lebih Besar dari sisa SPD", function() {
            if (nilaiangg < 0) {
                $("#nilaiSpdX" + idakun).val(0);//$("#nilaiSpdX" + idakun).autoNumeric('set', 0);
            } else {
                $("#nilaiSpdX" + idakun).val(accounting.formatNumber(nilaiangg, 2, '.', ","));//$("#nilaiSpdX" + idakun).autoNumeric('set', nilaiangg);
                /*
                if (nilaiSpd < nilaiSPDYangSudahDiserap) {
...
function validasinilaispp(idakun) {
    ...
    var nilaiSpd = eval(accounting.unformat($("#nilaiSpdX" + idakun).val(), ",")); //parseFloat($("#nilaiSpdX" + idakun).autoNumeric('get')); 
    var nilaiSPDYangSudahDiserap = $("#nilaiSPPcek" + idakun).val(); //$("#nilaiSPDYangSudahDiserap").val();
