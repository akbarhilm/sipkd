$(document).ready(function() {
    ...
    loadupdate(getbasepath() + "/spd/pengajuanbiaya/json/getlistspddetailbyskpddantahun");
    statusbreadcrum();
...
function getpagudanspd(id) {
    ...
    if (noSpd && $.trim(noSpd).length > 0) {
        ...
        gridspdbtllist(getbasepath() + "/spd/pengajuanbiaya/json/getlistspddetailbyskpddantahun", id, idSpd, tahunAnggaran);
    }
    $.getJSON(getbasepath() + "/spd/pengajuanbiaya/json/getanggarandanspd/" + id,
            function(result) {
...
function simpan() {
    ...
    var idSpd = $("#idSpd").val();
    var tahunAnggaran = $("#tahunAnggaran").val();
    submispddetail(getbasepath() + '/spd/pengajuanbiaya/json/addspddetail', getbasepath() + "/spd/pengajuanbiaya/json/getlistspddetailbyskpddantahun", idskpd, idSpd, tahunAnggaran);
}
function pindahhalamanadepan() {
    if ($("#idskpd").val().trim()) {
        window.location.replace(getbasepath() + "/spd/pengajuanbiaya/biaya/indexskpd/" + $("#idskpd").val().trim());
    } else {
        window.location.replace(getbasepath() + "/spd/pengajuanbiaya/biaya/index");
    }
}
...
function pasangvalidatebesarperfield(idakun) {
    ...
    var status = parseFloat(nilaiSpd) >= parseFloat(nilaiangg) ? false : true;
    console.log("nilaiSpd = "+nilaiSpd);
    console.log("nilaiangg = "+nilaiangg);
