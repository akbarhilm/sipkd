...
function getlistspdbtlbantuan(id) {
    $.getJSON(getbasepath() + "/spd/pengajuanbtlbantuanrev/json/getpagudansisabtlbantuan/" + id,
            function(result) {
...
function pindahhalamanadd() {
    if ($("#idskpd").val().trim()) {
        window.location.replace(getbasepath() + "/spd/pengajuanbtlbantuanrev/btlbantuan/addpengajuanspdbtlbantuanskpd/" + $("#idskpd").val().trim());
    } else {
        window.location.replace(getbasepath() + "/spd/pengajuanbtlbantuanrev/btlbantuan/addpengajuanspdbtlbantuan");
    }
}

function gridspdbiayamasterlist() {
    var urljson = getbasepath() + "/spd/pengajuanbtlbantuanrev/btlbantuan/json/getlistspd";
    ...
    if (typeof myTable == 'undefined') {
        myTable = $('#btlspdtable').dataTable({
            ...
            "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull, oSettings) {
                ...
                var urledit = baseurl + "/spd/pengajuanbtlbantuanrev/btlbantuan/editrev";
                var urldel = "#"
                ...
                $('td:eq(6)', nRow).html(aData['status'] == 'PENGAJUAN REVISI (APBDP)' ? editadd : '-');
                return nRow;
            },
            "aoColumns": [
