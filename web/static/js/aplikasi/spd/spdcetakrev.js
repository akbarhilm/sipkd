...
function trigercetak() {
    ...
    if (statuscek == 0) {
        $.ajax({
            type: "POST",
            url: baseurl + "/spd/pengajuancetakrev/json/insertspdcetak",
...
function gridspdbtlcetaklist() {
    var urljson = getbasepath() + "/spd/pengajuancetakrev/json/getlistspdcetak";
    if (typeof myTable == 'undefined') {
        myTable = $('#btlspdtable').dataTable({
            ...
            "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull, oSettings) {
                ...
                //alert(inputhiddennipPA);
                // alert(inputhiddennamaPA);
                // alert(inputhiddenrkPA);
                ...
                if (jen == 'BTL' )
                {
                     namafile = "SPDRev" + aData['JENIS'] + "-" + aData['C_ANGG_TAHUN'] + "-" + aData['NOSPD'] + ".pdf"
                }else if (jen == 'BTL BANTUAN')
                {
                     namafile = "SPDRev" + aData['JENIS'] + "-" + aData['C_ANGG_TAHUN'] + "-" + aData['NOSPD'] + ".pdf"
                }else if (jen == 'BL')
                ...
                if (aData['STATUS'] == 'PENGAJUAN REVISI (APBDP)') {
                ...
                } else if (aData['STATUS'] == 'CETAK REVISI (APBDP)') {
                ...
                }else if (aData['STATUS'] == 'VALIDASI REVISI (APBDP)') {
                    cekprint = "-";
                    donlod = "-";
                    batal = "-";
                    // $("div.top").html("");
...
function pindahhalcetakakun() {
    ...
    $("#popupcetakakun").prop("href", getbasepath() + "/spd/pengajuancetakrev/reports/pdf/");
...
function batalspd(idspd, nospd) {
    ...
    var urlhapusspd = getbasepath() + "/spd/pengajuancetakrev/json/batalspdcetak";
    bootbox.confirm("Anda akan membatalkan cetak data SPD APBDP dengan nomor " + nospd + " di " + $("#skpd").val() + ",  lanjutkan ? ", function(result) {
        if (result) {
            return   $.ajax({
            type: "POST",
            url:urlhapusspd ,
