...
function gridspdbtlvalidasilist() {
    ...
    var urljson = baseurl + "/spd/pembatalanspd/json/getlistspdvalidasi";
    ...
    if (typeof myTable == 'undefined') {
        myTable = $('#btlspdtable').dataTable({
            ...
            "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull, oSettings) {
                ...
                $('td:eq(1)', nRow).html(aData['noSpd'] + nospd+inputhiddennilaispd);
                ...
                var pilih="<a class='icon-remove' href='"+getbasepath()+"/spd/pembatalanspd/addbatal/"+aData['idSpdSah'] + "'  title='Klik disini untuk melakukan pembatalan SPD'></a>";
...
function submitvalidasi( ) {
    ...
    bootbox.confirm("Anda akan melakukan pengesahan untuk SPD dengan nomor " + arrnospd.join(",") + " di SKPD " + $("#skpd").val() + ",  lanjutkan ? ", function(result) {
        if (result) {
            return      $.ajax({
                type: "POST",
                url: baseurl + "/spd/pembatalanspd/json/submitvalidasi",
...
function printUrl(idspd) {
    var url = getbasepath() + "/spd/pembatalanspd/printvalidasi/cetakpengesahan/" + idspd;
    printPage(url)
