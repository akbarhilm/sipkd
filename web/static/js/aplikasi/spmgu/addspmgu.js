...
function getbanyakspdrinci( ) {
    $.getJSON(getbasepath() + "/sppgu/json/getlistspdblbanyak", {tahunAnggaran: $("#tahun").val(), idskpd: $("#idskpd").val()},
    function(result) {
...
function pindahhalamanadepan() {
    ...
    if (idskpd) {
        window.location.replace(getbasepath() + "/spmgu/indexspmgu/" + idskpd);
    } else {
        window.location.replace(getbasepath() + "/spmgu/indexspmgu");
    }
