...
var nobkumohon, jenispajak, triwulan;
...
function grid() {
    ...
    if (typeof myTable == 'undefined') {
        myTable = $('#skpdtable').dataTable({
            ...
            "bFilter": false,
            "sAjaxSource": getbasepath() + "/bkubos/json/listpajakpn",
            "aaSorting": [[1, "asc"]],
            "fnServerParams": function (aoData) {
                aoData.push(
                        {name: "tahun", value: $('#tahun').val()},
                        {name: "nobkumohon", value: nobkumohon},
                        {name: "jenispajak", value: jenispajak},
            ...
            "fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull, oSettings) {
                ...
                //idBku,noBkuMohon, noDok, uraian, nilaiKeluar, idKegiatan, kodeKeg, namaKeg, masaPajak, tahunPajak
                ...
                var npwp = "<input type='hidden' id='npwp" + index + "' value='" + aData['npwp'] + "' />";

                var kegiatan = "<textarea id='ketkeg" + index + "'readonly style='border:none;margin:0;width:500px;'>" + aData['kodeKeg'] + "/" + aData['namaKeg'] + "</textarea>";
                ...
                $('td:eq(0)', nRow).html(index + nobku + idbku + npwp);
                $('td:eq(3)', nRow).html(kegiatan + idkeg + kodekeg);
                $('td:eq(4)', nRow).html(nilaipajak + masapajak + tahunpajak);
