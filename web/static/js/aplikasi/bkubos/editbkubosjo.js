$(document).ready(function() {
    ...
    if ($('#kodeSaldoAwal').val() == "1") {
        document.getElementById("cbSaldoAwal").checked = true;
    }
});
...
function update() {
    ...
    var kodesaldo = $("#kodeSaldoAwal").val();

    var kas = accounting.unformat($("#saldokas").val(), ",");

    var varurl = getbasepath() + "/bkubos/json/prosesupdatebku";
    var dataac = [];
...
function getSaldoKasJO() {
    ...
    $.getJSON(getbasepath() + "/bkubos/json/getSaldoKasJO", {idbku: idbku, idsekolah: idsekolah, triwulan: triwulan},
    function(result) {
