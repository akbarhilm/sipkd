...
function checkNilai(noBkuMohon, callback) {
    ...
    $.getJSON(getbasepath() + "/statusbos/json/validateNilai", {idSekolah: idSekolah,
        triwulan: triwulan, noBkuMohon: noBkuMohon},
    function(result) {
        if (result != 0) {
            banyak += 1;
            //console.log(" banyak (if) = " + banyak);
...
function checkNilai2(noBkuMohon) {
    ...
    function(result) {
        banyak += result;
        //console.log("banyak di dalam " + banyak);
    });
...
function prosesAjukan() {
    var varurl = getbasepath() + "/statusbos/json/prosesupdatemohon";
    var dataac = [];
    var statusSesudah = '1';
...
function showGridPengajuan() {
    ...
    var urljson = getbasepath() + "/statusbos/json/listpengajuan";
