...
function getTutupBuku() {
    ...
    $.getJSON(getbasepath() + "/statusbos/json/getTutupBuku", {tahun: tahun,
...
function prosesSetujui() {
    var varurl = getbasepath() + "/statusbos/json/prosesupdate";
    var dataac = [];
    var data = {tahun: $("#tahun").val(), idSekolah: $("#idSekolah").val(),
        triwulan: $("#triwulan").val(), statusSebelum: '1', statusSesudah: '2', kodeTrx: 'JJ'}
    dataac = data;
    $.ajax({
        type: "POST",
        url: varurl,
...
function prosesBatalkan() {
    ...
    var dataac = [];
    var data = {tahun: $("#tahun").val(), idSekolah: $("#idSekolah").val(),
        triwulan: $("#triwulan").val(), statusSebelum: '2', statusSesudah: '1', kodeTrx: 'JJ'}
    dataac = data;
    $.ajax({
        type: "POST",
        url: varurl,
...
function showGrid() {
    ...
    var urljson = getbasepath() + "/statusbos/json/listpersetujuan";
    myTable = $('#mytable').dataTable({
        ...
        "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull, oSettings) {
            ...
            var isChecked = aData['status'] == '2' ? 'checked' : '';
            var cekStatus = "<input type='checkbox' name='isapproved" + index + "' id='isapproved" + index + "' disabled  " + isChecked + " />"
...
function getTotal() {
    ...
    $.getJSON(getbasepath() + "/statusbos/json/getTotalDari", {tahun: tahun,
        idSekolah: idSekolah, triwulan: triwulan, kodeTrx: 'JJ', dari: '1', hingga: '2'},
    function(result) {
...
function getBanyak() {
    ...
    $.getJSON(getbasepath() + "/statusbos/json/getBanyak", {tahun: tahun,
