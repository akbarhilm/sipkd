...
function ambilskpd(id) {
   // console.log($("#namaSkpd" + id).val())
    $('#skpd', window.parent.document).val($("#namaSkpd" + id).val());
  //  $('#levelSkpd', window.parent.document).val($("#levelSkpd")).val();
    $('#idskpd', window.parent.document).val(id).change();
    
    parent.$.fancybox.close();
}
function grid( ) {
    var urljson = getbasepath() + "/common/json/listskpdjsonall";
    $("#skpdtable").show();
    if (typeof myTable == 'undefined') {
        myTable = $('#skpdtable').dataTable({
            ...
            "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull, oSettings) {
                ...
                //alert(l);
                var textnamaskpd = "<input type='hidden' id='namaSkpd" + aData['idSkpd'] + "' value='" + aData['kodeSkpd'] + " / " + aData['namaSkpd'] + " / " + aData['levelSkpd'] + "' />";
                //alert(textnamaskpd);
