...
function update() {
    if (jumbarisspj > 0) {
        ...
        var varurl = getbasepath() + "/koreksinilaispjbop/json/prosesupdatekoreksi";
        var dataac = [];
        var nilailist = [];
        var i;
        var banyak = 0;
...
function hapus() {
    ...
    bootbox.confirm("Anda akan menghapus data koreksi ini, lanjutkan ? ", function(result) {
        if (result) {
            var varurl = getbasepath() + "/koreksinilaispjbop/json/prosesdeletebyid";
            var dataac = [];
...
function getPaguEdit() {
    ...
    $.getJSON(getbasepath() + "/koreksinilaispjbop/json/getPaguEdit", {idsekolah: $('#idsekolah').val(), nomohon: $('#noBkuRef').val()},
    function(result) {
