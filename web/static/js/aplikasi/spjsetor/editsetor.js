...
function cekNilaiSA(nilai) {
    ...
    if (accounting.unformat(nilai, ",") > saldo) {
...
function getdatakegiatan() {
    ...
    function(result) {
        var kegiatan = result.aData[0]['kodeKeg'] + "/" + result.aData[0]['namaKeg'];

        if ($('#tahunAngg').val() >= 2017) {
            $('#namakegiatan').val(kegiatan);
...
function getbanyaksetoredit() {
    ...
    $.getJSON(getbasepath() + "/setor/json/getlistsetoreditbanyak", {idskpd: $("#idskpd").val(), idKegiatan: $("#idkegiatan").val(), idkegiatan: $("#idkegiatan").val(), tahunAnggaran: $("#tahunAngg").val(), idSetor: $("#idSetor").val()},
    function(result) {
...
function gridedit() {
    ...
    if (typeof myTable == 'undefined') {
        myTable = $('#forrinci2016').dataTable({
            "bPaginate": true,
