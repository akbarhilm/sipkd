...
function cekDataBank() {
    ...
    if ($('#dkinama').val().trim() !== $('#namaTujuan').val()) {
        datasama = false;
        if (keterangan == "Data ") {
...
function cek() {
    ...
    if ($("#kodeBank").val() == "111") {
        ...
        namatujuan = $("#namaTujuan").val().substr(0, 30);
    }
    ...
    } else if ($("#kodePkp").val() !== $("#pkp").val()) {
        bootbox.alert("Status PKP Telah Berubah, Silahkan Cek Kembali Potongan SPM Anda");
    } else {
        return true;
    }
...
function getDataNPWP() {
    var varurl = getbasepath() + "/spmterima/json/inquirynpwp";
    var dataac = [];
    var datajour = {
        npwp: $('#noNpwp').val()
    };
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            if (result['error'] == null) {
                $("#namawp").val(result['NAMA']);
                $("#alamatwp").val(result['ALAMAT']);
                $("#kotawp").val(result['KABKOT']);
                $("#pkp").val(result['STATUS_PKP']);
            } else {
