$(document).ready(function () {
    ...
    $("#nospmfilter").keyup(function() {
        var panjang = $(this).val().length;
        if (panjang > 0 || panjang == 0) {
            gridterimaspm();
        }
    });
...
function cekDataBank() {
    ...
    if ($('#dkinamabank').val() !== $('#namaBank').val()) {
        datasama = false;
        if (keterangan == "Data ") {
...
/*
    if ($('#dkinama').val() !== $('#namaRekanan').val()) {
        datasama = false;
        if (keterangan == "Data ") {

            keterangan = keterangan + "Nama Rekanan";
        } else {
            keterangan = keterangan + " , Nama Rekanan";
        }
    ...
    if ($('#dkinpwp').val() !== $('#npwpRekanan').val()) {
        datasama = false;
        if (keterangan == "Data ") {
            keterangan = keterangan + "NPWP";
        } else {
            keterangan = keterangan + " , NPWP";
        }
        ...
        myTable = $('#terimaspmtable').dataTable({
            ...
            "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull, oSettings) {
                ...
                switch (aData['codeWilSp2dproses']){
                    ...
                    case "0" : codeWilSp2dproses = "Abdul Muis"; break;
                }
