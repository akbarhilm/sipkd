...
function gridsp2drincibtlls() {
    var urljson = getbasepath() + "/sp2dbantuanls/json/getlistsp2drincibantuanls";
    if (typeof myTable == 'undefined') {
        myTable = $('#btllssp2drincitable').dataTable({
            ...
            "fnServerParams": function (aoData) {
                ...
                {name: "idskpdkoord", value: $('#idskpdkoord').val()}
                );
            }, "fnFooterCallback": function (nRow, aaData, iStart, iEnd, iDisplay) {
                var sp2dTotal = 0;
                if (aaData.length > 0) {
                    for (var i = iStart; i < iEnd; i++) {
                        sp2dTotal += parseFloat(aaData[i]['nilaiSpm']);
                    }
                }
            ...
            "fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull, oSettings) {
                ...
                var nilaisp2d = accounting.formatNumber(aData['nilaiSpm'], 2, '.', ",");
                var nilaiAnggaran = accounting.formatNumber(aData['nilaiAnggaran'], 2, '.', ",");

                $('td:eq(0)', nRow).html(index);
                $('td:eq(5)', nRow).html(nilaiAnggaran);
                $('td:eq(6)', nRow).html(nilaisp2d);
                return nRow;
            },
            "aoColumns": [
                ...
                {"mDataProp": "nilaiAnggaran", "bSortable": false, sClass: "kanan", "sWidth": "20%"},
