...
function tambahRow() {
    ...
    cell6.innerHTML = "<td> <input type='text' name='debet" + idrow + "' id='debet" + idrow + "' class='inputmoney' readonly='true'  onkeyup='getTotal(),validasiDebet(" + idrow + ")' onclick = 'validasiDebet(" + idrow + ")' /> </td>";
    cell7.innerHTML = "<td> <input type='text' name='kredit" + idrow + "' id='kredit" + idrow + "' class='inputmoney' readonly='true'  onkeyup='getTotal(),validasiKredit(" + idrow + ")' onclick = 'validasiKredit(" + idrow + ")' /> </td>";
...
function getnama(kode, nama) {
    $.getJSON(getbasepath() + "/journalppkd/json/getnamaakun", {kode: kode},
    function(result) {
        var id = nama;
...
function getnojurnal(tahun) {
    var tahun = $("#tahunAngg").val();
    $.getJSON(getbasepath() + "/journalppkd/json/getnojurnal", {tahun: tahun},
    function(result) {
...
function update() {
    ...
    if (tkredit == tdebet) {
        if (tglpost == "") {
            ...
            var varurl = getbasepath() + "/journalppkd/json/prosessimpanupdate";
            var dataac = [];
            var nilainr = [];
            var nilailist = [];
            var nilainojurnal = [];
            ...
            return   $.ajax({
                ...
                clearrow();
                bootbox.alert("Data Jurnal Umum PPKD Berhasil Disimpan");
            });
        }
    } else {
    ...
    if (idrow > 0) {
        ...
        if (tkredit == tdebet) {
            if (tglpost == "") {
                ...
                var varurl = getbasepath() + "/journalppkd/json/prosessimpan";
                var dataac = [];
                var nilainr = [];
                var i;
                var p = 0;
...
function getbanyakrinci() {
    $.getJSON(getbasepath() + "/journalppkd/json/getlistbanyakrinci", {noJournal: $("#noJournal").val()},
    function(result) {
        banyakdata = result;
...
function getrincilist() {
    $.getJSON(getbasepath() + "/journalppkd/json/valtabel", {noJournal: $("#noJournal").val()},
    function(result) {
        var i, c, jenis, beban;
        var td = 0;
        var tk = 0;
...
function hapusdata() {
    if (statusupdate == 1) {
        ...
        bootbox.confirm("Anda akan menghapus data Jurnal Umum PPKD dengan nomor " + $("#noJournal").val() + " tahun " + $("#tahunAngg").val() + ",  lanjutkan ? ", function(result) {
            if (result) {
                //bootbox.alert("Masuk hapus data");

                return   $.ajax({
                    type: "POST",
                    url: getbasepath() + "/journalppkd/json/prosesupdateaktif",
                    contentType: "text/plain; charset=utf-8",
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type': 'application/json'
...
function tampilCek() {
    //document.getElementById("koreksiBPK").checked

    //$("#koreksidariBPK ").checked 
    //document.getElementById("cekpilih" + baris).checked
    if (document.getElementById("koreksidariBPK").checked == true) {
    ...
    console.log("ket koreksi = " + $("#ketkoreksi").val());
    //bootbox.alert("Status update = " + statusupdate);
