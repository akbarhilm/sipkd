...
function hitungnilaispj() {
    ...
    var kode = $('#kodeBeban').val();
    console.log("kode beban = " + kode);
    //if (kode == "UP/GU" || kode == "UP" || kode == "GU") {
    if (kode !== "TU") {
        var searchIDs = $("#editkegiatanUPtable input:checkbox:checked").not("#pilihall").map(function() {
            return $(this).val();
        }).get();
        var total = 0;
        $.each(searchIDs, function(idx, data) {
            total += parseFloat(accounting.unformat($("#nilaispj" + data).val(), ","));
            console.log("nilaispj +  " + data);
        })
        ...
        var searchIDs = $("#editkegiatanTUtable input:checkbox:checked").not("#pilihall").map(function() {
            return $(this).val();
        }).get();
        var total = 0;
        $.each(searchIDs, function(idx, data) {
...
function ubahnilaispj( ) {
    ...
    if (total > sisa_uang && beban != "TU") {
        bootbox.alert("Total SPJ Tidak Boleh Lebih Besar dari Sisa Uang UP/GU Saat Ini");
        ...
        $(".cekpilih:checked").each(function() {
            var datapeg = {
                ...
                addeditval: $("#addeditval" + $(this).val()).val(),
        ...
        return   $.ajax({
            ...
            var kode = $('#kodeBeban').val();
            console.log("Cek kode beban = " + kode);
            cekbeban(kode);
            //gridspjrincikegiatan();
...
function gridspjrincikegiatanedit() {
    ...
    if (typeof myTable2 == 'undefined') {
        myTable2 = $('#editkegiatanTUtable').dataTable({
            "bPaginate": true,
...
function cekbeban(beban) {
    console.log(" cekbeban() beban ===  " + beban);

    if (beban == "UP/GU" || beban == "UP" || beban == "GU") {
        $('#editkegiatanTUtable').hide();//hide();
        $('#editkegiatanUPtable').show();
        document.getElementById("labelsisaTU").style.display = "none";
        gridspjrincikegiataneditupgu();

    } else if (beban == "TU") { // TU
        $('#editkegiatanTUtable').show();
        $('#editkegiatanUPtable').hide();
        document.getElementById("labelsisaTU").style.display = "none"; // ga jadi
        gridspjrincikegiatanedit();
        //gettotalpagutu();
    }
...
function gridspjrincikegiataneditupgu() {
    //bootbox.alert("Kode beban #kodeBeban == "+ $('#kodeBeban').val());
    var urljson = getbasepath() + "/spj/json/getlistspjrincikegiatan";
    if (typeof upgutable == 'undefined') {
        upgutable = $('#editkegiatanUPtable').dataTable({
            "bPaginate": true,
            "sPaginationType": "bootstrap",
            "bJQueryUI": false,
            "bProcessing": true,
            ...
            "bScrollAutoCss": true,
            "bFilter": false,
            "aLengthMenu": [[1, 50, 100, 200, -1], [1, 50, 100, 200, "All"]],
            "iDisplayLength": 200,
            "sAjaxSource": urljson,
            "aaSorting": [[2, "asc"]],
            "fnDrawCallback": function() {
            ...
            "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull, oSettings) {
                ...
                var addeditval = "<input type='hidden' name='addeditval" + aData['idBl'] + "' id='addeditval" + aData['idBl'] + "' value='" + ketaddedit + "'   />"
                ...
                $('td:eq(8)', nRow).html(inputcek + idbas + idbl + addeditval);
                return nRow;
            },
            "aoColumns": [
                {"mDataProp": "akun.kodeAkun", "bSortable": false, sClass: "center", "sWidth": "5%"},
...
function gettotalpagutu() {
    var idskpd = $("#idskpd").val();
    var tahun = $("#tahun").val();
    $.getJSON(getbasepath() + "/spj/json/gettotalpagutu", {idskpd: idskpd, tahun: tahun, idkegiatan: $('#idKegiatan').val(), idspj: $('#idspj').val()},
    function(result) {
        var sisa_uang = result.aData['sisaUang'];
        var totspp = result.aData['totalSpp'];
        var totspj = result.aData['totalSpj'];

        $("#sisaUangTU").val(accounting.formatNumber(sisa_uang, 2, '.', ","));
