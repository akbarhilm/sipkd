$(document).ready(function() {
    ...
    jumlahawal = $('#sumspj').val();
    console.log("jumlah awal = " + jumlahawal);


    console.log($('#kodeTalangan').val());
...
function setSpj() {
    ...
    function(result) {
        ...
        if (result > 0) { // SPJ bisa input tapi akan cek, jika sudah pernah mengajukan maka v_spj_mohon tidak diisi
            statusspj = 1; // update spj sesudah diajukan ke sudin
            getSisaKasSpjTambahan();
            document.getElementById("labelsisaspj").style.display = "block";

        } else {
            statusspj = 0; // update spj sebelum diajukan ke sudin
...
function getDataKegiatan() {
    ...
    $.getJSON(getbasepath() + "/bkubop/json/getDataKegiatan", {tahun: tahun, idkegiatan: idkegiatan},
    function(result) {
...
function gridspj() {
    jumbarisspj = 0;
    console.log("jumbaris spj = " + jumbarisspj);
    var bukti = $('#bukti').val();
    if (typeof myTable == 'undefined') {
        myTable = $('#spjtable').dataTable({
            ...
            "bFilter": false,
            "sAjaxSource": getbasepath() + "/bkubop/json/listspjedit",
            "aaSorting": [[1, "asc"]],
            ...
            "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull, oSettings) {
                ...
                var akun = "<textarea id='akun" + index + "'readonly style='border:none;margin:0;width:170px;'>" + aData['namaakun'] + "</textarea>";
                var komponen = "<textarea id='komponen" + index + "'readonly style='border:none;margin:0;width:170px;'>" + aData['namaKomponen'] + "</textarea>"; // aData['kodeKomponen'] + "/" +
                ...
                if (bukti == '1') {
                    var nilaiinput = "<input type='text' readonly='true' name='nilaiinput" + index + "' id='nilaiinput" + index + "'  class='inputmoney'  value='" + aData['nilaiBkuInput'] + "' onkeyup='pasangvalidatebesarperfield(" + index + ")' onchange='pasangvalidatebesarperfield(" + index + "); validasiNilaiPajak(" + index + ")' />";
                } else {
                ...
                $('td:eq(1)', nRow).html(akun + idbas + textkodeakun + paguakun + nilaisebelumtw);
            ...
            "aoColumns": [
                ...
                {"mDataProp": "nilaiSisa", "bSortable": false, sClass: "right", "sWidth": "5%"},
...
function simpan() {
    ...
    if ((jum3 > 0) && ($("#akunnama1").val() !== "")) {
        ...
        if ((sk == "1" && kodesk == "") || nrkPptk == "" || nobukti == "" || tglDok == "" || filling == "" || idKegiatan == "" || nippptk == "" || namapptk == "" || namarekan == "" || bank == "" || norek == "" || (npwp != "" && namanpwp == "")) {
...
function update() {
    if (jumbarisspj > 0) {
        ...
        var varurl = getbasepath() + "/bkubop/json/prosesupdatebkuspj";
        var dataac = [];
        var nilailist = [];
        var i;
        var banyak = 0;
...
function updatebukti() {
    if (jumbarisspj > 0) {
        ...
        var varurl = getbasepath() + "/bkubop/json/prosesupdatebkuspjbukti";
        var dataac = [];
