...
function simpan() {
    ...
    if (nrk == "" || nobukti == "" || tglDok == "" || filling == "" || idKegiatan == "" || nippptk == "" || namapptk == "" || namarekan == "" || ($("#jenisPembayaran").val() == "PN" && npwp == "") || $('#tahunPajak').val() == "" || (npwp != "" && namanpwp == "")) {
    ...
    } /*else if (parseInt($("#nilaiinput1").val()) == 0) {
     bootbox.alert("Nilai Pajak Harus Diisi");
...
function update() {
    ...
    if (jumlah > 0) {
        var varurl = getbasepath() + "/bkubop/json/prosesupdatepajak";
        var dataac = [];
        var nilailist = [];
        var i;
        var banyak = 0;
...
function hapus() {
    ...
    bootbox.confirm("Anda akan menghapus data BKU ini, lanjutkan ? ", function(result) {
        if (result) {
            var varurl = getbasepath() + "/bkubop/json/prosesdeletepjpn";
            var dataac = [];
            var nilailist = [];
            var jumlah, i;
            ...
            return   $.ajax({
                ...
                console.log("status proc update pajak = " + data);
