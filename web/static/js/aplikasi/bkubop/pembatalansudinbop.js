...
function prosesSetujui() {
    ...
    $(".ischecked:checked").each(function() {
        ...
        var data = {tahun: $("#tahun").val(), idSekolah: $("#idSekolah").val(),
            triwulan: $("#triwulan").val(), statusSebelum: '3', statusSesudah: '0', kodeTrx: 'JJ', noMohon: noMohon}
        dataac.push(data);
    });
...
function prosesSetujuiAll() {
    ...
    var dataac = [];
    var data = {tahun: $("#tahun").val(), idSekolah: $("#idSekolah").val(),
        triwulan: $("#triwulan").val(), statusSebelum: '3', statusSesudah: '0', kodeTrx: 'JJ'}
    dataac = data;
    $.ajax({
        type: "POST",
        url: varurl,
...
function prosesBatalkan() {
    ...
    $(".ischecked:checked").each(function() {
        ...
        var data = {tahun: $("#tahun").val(), idSekolah: $("#idSekolah").val(),
            triwulan: $("#triwulan").val(), statusSebelum: '2', statusSesudah: '1', kodeTrx: 'JJ', noMohon: noMohon}
        dataac.push(data);
    });
...
function showGrid() {
    ...
    var total = parseInt($('#sudin').val()) + parseInt($('#belum').val());
...
function getTotal() {
    ...
    $.getJSON(getbasepath() + "/statusbop/json/getTotalDari", {tahun: tahun,
        idSekolah: idSekolah, triwulan: triwulan, kodeTrx: 'JJ', dari: '3', hingga: '0'},
    function(result) {
