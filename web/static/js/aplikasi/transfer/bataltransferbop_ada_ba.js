...
function cekPajakPengeluaran(noBkuMohon, callback) {
    ...
    $.getJSON(getbasepath() + "/btltf/json/getPajakPengeluaran", {idSekolah: idSekolah,
        triwulan: triwulan, noBkuMohon: noBkuMohon, kodesumberdana: '2'},
    function(result) {
        if (result != 0) {
            callback(true);
        } else {
...
//        kodeotor: $("#kodeOtoritas").val(), kodesumberdana: '2', batal: '1'};
//    dataac = data;
//    $.ajax({
//        type: "POST",
//        url: varurl,
    ...
    $(".ischecked:checked").each(function() {
        ...
        if ($('#idba' + id).val() > 0) {
            ...
            var data = {idba: $('#idba' + id).val(), idSekolah: $("#idSekolah").val(), triwulan: $("#triwulan").val(),
                kodeotor: $("#kodeOtoritas").val(), kodesumberdana: '2', noBkuMohon: noBkuMohon, batal: '1', kodetransaksi: $("#kodetransaksi").val()};
            dataac.push(data);
            count++;
    ...
    $.ajax({
        ...
        if ($("#kodeOtoritas").val() == '2') {
            if (count == 0) {
                ...
                bootbox.alert($("#kodetransaksi option:selected").text() + " sudah diajukan sebelumnya");
            } else {
...
function prosesPajak() {
    ...
    $(".ischeckedpajak:checked").each(function() {
        ...
        if ($('#idbapajak' + id).val() > 0) {
            ...
            console.log("NO MOHON X " + noBkuMohon);
            var data = {idba: $('#idbapajak' + id).val(), idSekolah: $("#idSekolah").val(), triwulan: $("#triwulan").val(),
                kodeotor: $("#kodeOtoritas").val(), kodesumberdana: '2', noBkuMohon: noBkuMohon, batal: '1'};
            dataac.push(data);
            count++;
...
//function batalAll() {
//    var varurl = getbasepath() + "/bkutf/json/prosesbatalsemua";
//    var dataac = [];
//    var data = {idSekolah: $("#idSekolah").val(), triwulan: $("#triwulan").val(),
//        kodeotor: $("#kodeOtoritas").val(), kodesumberdana: '2', batal: '0'};
//    dataac = data;
//    $.ajax({
//        type: "POST",
//        url: varurl,
...
function batal() {
    ...
    $(".ischecked:checked").each(function() {
        ...
        if ($('#idba' + id).val() > 0) {
            ...
            var data = {idSekolah: $("#idSekolah").val(), triwulan: $("#triwulan").val(),
                kodeotor: $("#kodeOtoritas").val(), kodesumberdana: '2', noBkuMohon: noBkuMohon, batal: '0', kodetransaksi: $("#kodetransaksi").val()};
            dataac.push(data);
            count++;
    ...
    $.ajax({
        ...
        data: JSON.stringify(dataac)}).always(function() {
        if ($("#kodeOtoritas").val() == '2') {
            if (count == 0 || countsudah > 0) {
                bootbox.alert($("#kodetransaksi option:selected").text() + " belum diajukan");
            } else {
...
function batalPajak() {
    ...
    $(".ischeckedpajak:checked").each(function() {
        ...
        if ($('#idbapajak' + id).val() > 0) {
            ...
            console.log("NO MOHON X " + noBkuMohon);
            var data = {idSekolah: $("#idSekolah").val(), triwulan: $("#triwulan").val(),
                kodeotor: $("#kodeOtoritas").val(), kodesumberdana: '2', noBkuMohon: noBkuMohon, batal: '0'};
            dataac.push(data);
            count++;
    ...
    $.ajax({
        ...
        if ($("#kodeOtoritas").val() == '2') {
            if (count == 0 || countsudah > 0) {
                bootbox.alert("Pajak belum diajukan");
            } else {
                bootbox.alert("Pengajuan pajak berhasil dibatalkan");
            }
        } else if ($("#kodeOtoritas").val() == '1') {
            if (countsudah > 0) {
...
function getTotal() {
    ...
    $.getJSON(getbasepath() + "/btltf/json/getTotal", {idSekolah: idSekolah, triwulan: triwulan,
        kodeotor: kodeotor, kodesumberdana: '2', kodetransaksi: kodetransaksi},
    function(result) {
...
function getJumlah() {
    ...
    $.getJSON(getbasepath() + "/btltf/json/getJumlah", {idSekolah: idSekolah,
        triwulan: triwulan, kodeotor: kodeotor, kodesumberdana: '2', kodetransaksi: kodetransaksi},
    function(result) {
