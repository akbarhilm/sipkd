...
function getToken() {
    ...
    $.getJSON(getbasepath() + "/token/json/getToken", {idsekolah: idsekolah, kodesumbdana: '2', noBkuMohon: noBkuMohon},
    function(result) {
        var data, kode, ket;
        data = result.aData;
        if (data != null) {
...
function validateTime() {
    $.getJSON(getbasepath() + "/token/json/getTimeLimit", {kodesumbdana: 2, nomohon: $('#noBkuMohon').val(), idsekolah: $('#idsekolah').val(), tahun: $('#tahun').val()},
    function(result) {
...
function setTujuan() {
    ...
    if (jenistransaksi == "ST" || jenistransaksi == "JG" || jenistransaksi == "RT") {
        $.getJSON(getbasepath() + "/bkutf/json/getDataBankTujuan", {idskpd: $('#idskpd').val()},
        function(result) {
    ...
    if (typeof myTable2 == 'undefined') {
        myTable2 = $('#spjtable').dataTable({
            ...
            "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull, oSettings) {
                ...
                var pilihpajak = "<a id='inputpajak" + index + "' onclick='setButton(" + index + ")' class='fancybox fancybox.iframe' href='" + getbasepath() + "/bkutf/listpajakbop?target='_top'' title='Input Pajak SPJ'  ><i class='icon-edit'>";
...
function getDataBankBOP() {

    $.getJSON(getbasepath() + "/bkutf/json/getDataBankBOP", {idsekolah: $('#idsekolah').val()},
    function(result) {
...
function cekbayar() {
    ...
    if (jenistransaksi.substr(0, 1) !== "P") { // untuk testing di bandung, validasi dilepas dl
        ...
        if (saldo < nilaibku && jenistransaksi == "JJ") {
            bootbox.alert("Saldo Kas Tidak Mencukupi untuk Melakukan Pembayaran SPJ");
        } else if ($("#token").val() == "0" && jenistransaksi.substr(0, 1) != "P") {
            bootbox.alert("Ajukan Token Terlebih Dahulu");
        } else if (jenistransaksi.substr(0, 1) == "P") {
            if ($("#tglPosting").val() == "") {
                bootbox.alert("Pengisian Data Harus Lengkap");
            ...
            if (count = 1) {
                ...
                $.getJSON(getbasepath() + "/bkutf/json/getKodeStan", {},
                        function(result) {
                            ...
                            var var62 = tahun.toString() + "3" + $("#kodeBatalTf").val() + idbku.toString(); // yyyy+ kode sumb + kode retur + idbku // kode app diganti untuk SP2D jadi 3 (asalnya 2) agar sama dengan urutan kode app di ws pajak (pertanggal 15 jan 19)
...
function updateBkuBank(idbku, status, trxterima, tglproses, nilai, msg) {
    ...
    var varurl = getbasepath() + "/bkutf/json/updatebkubank";
    var dataac = [];
    var tahun = $('#tahun').val();
    var datajour = {
        idsekolah: $("#idsekolah").val(),
...
function getNilaiSpjNetto() {
    ...
    $.getJSON(getbasepath() + "/bkutf/json/getNilaiSpjNettoBop", {tahun: tahun, idsekolah: idsekolah, idbku: idbku},
    function(result) {
