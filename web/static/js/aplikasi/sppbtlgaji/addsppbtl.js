$(document).ready(function () {
    ...
    //cekreplace();
    //document.getElementById("buttoninduk").style.visibility = 'hidden';//$('#buttoninduk').attr("disabled", true);
    if($('#kodesimpeg').val() != 0 || $('#kodesimpeg').val() != null){
        gridspprinci();
    }
    
    if($('#idlist').val() != -1 && $('#idlist').val() != null){
        getNilaiSPP();
    }
    var d = new Date();
    var m = d.getMonth()+1;
    ...
    if (typeof myTable == 'undefined' ) {
        myTable = $('#spprincitable').dataTable({
            ...
            "fnDrawCallback": function () {
                ...
                //$(".inputmoney").priceFormat({prefix: "", suffix: "", centsSeparator: ",", thousandsSeparator: ".", limit: 15, allowNegative: false});
            ...
            "fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull, oSettings) {
                ...
                var pilih = "<input type='hidden' id='pilih" + index + "'   name='pilih" + index + "' value='0' />";
                ...
                var nilaispptext = "<input type='text' name='nilaispp" + index + "' id='nilaispp" + index + "'  class='inputmoney'  readonly='true'  value='" + nilaispp + "'  onkeyup='pasangvalidatebesarperfield(" + index + ");hitungnilaispp()' />" //
            ...
            "aoColumns": [
                ...
                {"mDataProp": "noSpd", "bSortable": true, sClass: "center", "sWidth": "4%"},
                {"mDataProp": "akun.kodeAkun", "bSortable": true, "sWidth": "10%"},
                {"mDataProp": "akun.namaAkun", "bSortable": true, "sWidth": "22%"},
                {"mDataProp": "nilaiSpd", "bSortable": false, sClass: "kanan", "sWidth": "14%"},
                {"mDataProp": "nilaiSppSebelum", "bSortable": true, "sWidth": "14%"},
                {"mDataProp": "nilaiSppSisa", "bSortable": true, "sWidth": "14%"},
...
function getNilaiSPP(){
    ...
    $.getJSON(getbasepath() + "/btlgaji/json/getNilaiSPP/" + idlist + "/" + kdsimpeg,
            function (result) {
                ...
                console.log("JUMKOT DARI QUERY == "+result[0].JUMKOT);
                var i;
                ...
                hitungnilaispp();
                //tambahan jilid 3
                //ceknilaitotal(parseFloat(accounting.unformat($('#totalspp').val(), ",")),$('#jumkotsimpeg').val());
                /*
                if(parseFloat(accounting.unformat($('#totalspp').val(), ",")) < $('#jumkotsimpeg').val()){//nilai total <> nilai yg diinput
                    document.getElementById("buttoninduk").disabled = "disabled";//buttoninduk //matikan tombol simpan
                    bootbox.alert("Total dari Simpeg Lebih Besar dari Nilai Sisa SPD.",function () {
                        window.location.replace(getbasepath() + "/btlgaji/addsppbtl/");
                    }) ;//dikasih warning bahwa spd tidak cukup
                }
                */
            });
}

function ceknilaitotal(total, jumkot) {
    var idskpd = $('#idskpd').val();
    if (total < jumkot) {//nilai total <> nilai yg diinput
        document.getElementById("buttoninduk").disabled = "disabled";//buttoninduk //matikan tombol simpan
        bootbox.alert("Total dari Simpeg Lebih Besar dari Nilai Sisa SPD.", function() {
    ...
    for (i = 1; i <= banyakBarisGrid; i++) {
        ...
        if (kodeakun == $("#kodeAkun" + i).val() ) {
            ...
            if (parseFloat(nilai) == 0 || $("#statspd" + i).val() == 'P') {
                ...
                document.getElementById("cekpilih" + i).title = accounting.formatNumber(nilai, 2, '.', ",").toString();
                $("#pilih" + i).val("1");
                //$("#nilaispp" + i).val(accounting.formatNumber(nilai, 2, '.', ",")).change();
                $("#nilaispp" + i).autoNumeric('set', nilai);
                $("#nilaispp" + i).attr("readonly", false);
                break;
            }
            else {
                if (parseFloat(accounting.unformat($("#nilaiSppSisa" + i).val(), ",")) > 0) {
                    ...
                    document.getElementById("cekpilih" + i).title = $("#nilaiSppSisa" + i).val().toString();
                    //console.log("NILAI AKUN == " + nilai);
                    $("#pilih" + i).val("1");
                    //$("#nilaispp" + i).val($("#nilaiSppSisa" + i).val()).change();
                    $("#nilaispp" + i).autoNumeric('set', accounting.unformat($("#nilaiSppSisa" + i).val(), ","));
                    $("#nilaispp" + i).attr("readonly", false);
                }
            }
        }
...
function hitungnilaispp() {
    ...
    $.each(searchIDs, function (idx, data) {
        ...
        //total += parseFloat($("#nilaispp" + data).autoNumeric('get'));
...
function pasangvalidatebesarperfield(index) {
    ...
    if (!status) {
        bootbox.alert("Nilai SPP tidak boleh lebih besar dari nilai SPP Sisa", function () {
            console.log("nilaisisa = "+nilaisisa);
            $('#nilaispp' + index).autoNumeric('set', nilaisisa);// edit 9 Nov 2016 by zainab : format input money diganti
