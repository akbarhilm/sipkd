...
function setTriwulan() {
    ...
    $.getJSON(getbasepath() + "/koreksinilaispjbos/json/getListTriwulanByRekap", {tahun: tahun, idsekolah: idsekolah},
    function(result) {
        var banyak, kode, ket;
...
function gridspj() {
    ...
    if (typeof myTable == 'undefined') {
        myTable = $('#spjtable').dataTable({"bPaginate": true,
            ...
            "bFilter": false,
            "sAjaxSource": getbasepath() + "/koreksinilaispjbos/json/listspjkoreksi",
            "aaSorting": [[1, "asc"]],
            ...
            "fnFooterCallback": function(nRow, aaData, iStart, iEnd, iDisplay) {
                ...
                for (var i = iStart; i < iEnd; i++) {
                    total += parseFloat(aaData[i]['nilaiBkuInput']);
...
function update() {
    if (jumbarisspj > 0) {
        ...
        var varurl = getbasepath() + "/koreksinilaispjbos/json/prosesupdatekoreksi";
        var dataac = [];
        var nilailist = [];
        var i;
        var banyak = 0;
...
function hapus() {
    ...
    bootbox.confirm("Anda akan menghapus data koreksi ini, lanjutkan ? ", function(result) {
        if (result) {
            var varurl = getbasepath() + "/koreksinilaispjbos/json/prosesdeletebyid";
            var dataac = [];
            ...
            return   $.ajax({
                ...
                window.location.href = getbasepath() + "/koreksinilaispjbos/index"; // ke form index
            });
...
function getPaguEdit() {

    $.getJSON(getbasepath() + "/koreksinilaispjbos/json/getPaguEdit", {idsekolah: $('#idsekolah').val(), nomohon: $('#noBkuRef').val()},
    function(result) {

        $("#pagukoreksi").val(accounting.formatNumber(result.aData['nilaiSpj'], 2, '.', ","));
