<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<ul class="breadcrumb">
    <li>
        <i class="icon-home"></i>
        <a href="${pageContext.request.contextPath}/beranda">SP2D</a>
        <span class="icon-angle-right"></span>
    </li>
    <li><a href="#"> Pembayaran Pajak SP2D </a></li>
</ul>
<div class="portlet box red">
    <!--  <div class="portlet-title">
          <div class="caption"><i class="icon-cogs"></i>Register  SP2D</div>
          <div class="actions">
              <a href="#" onclick="pindahhalamanadd()" class="btn dark"><i class="icon-plus"></i> Tambah</a>
          </div>
      </div>-->
    <div class="portlet-body flip-scroll">
        <form class="form-horizontal">
            <div class="form-group">
                <label class="col-md-4 control-label">Nama SKPD:</label>
                <div class="col-md-8">
                    <input type="hidden" id="idskpd" name="idskpd"/>
                    <input type="hidden" id="kproses" name="kproses"  />
                    <input type="hidden" id="tahun"  name="tahun"  />
                    <input type="hidden" id="idsp2d"  name="idsp2d" />
                    <input type="text"  name="skpd"  id="skpd"  class="m-wrap large" size="70"    readonly="true" />
                </div>
            </div>
            <div class="form-group">
                <label class="col-md-4 control-label">No SP2D:</label>
                <div class="col-md-8">
                    <input type="text"  name="nosp2d"  id="nosp2d"  class="m-wrap large" size="70"  readonly="true" />

                </div>
            </div>
            <div class="form-group">
                <label class="col-md-4 control-label">Tanggal SP2D:</label>
                <div class="col-md-8">
                    <input type="text"  name="tglsp2d"  id="tglsp2d"  class="m-wrap large" size="70"  readonly="true" />

                </div>
            </div>
            <div class="form-group">
                <label class="col-md-4 control-label">Tahun:</label>
                <div class="col-md-8">
                    <div class="input-group">
                        <input type="text" name="tahun" id="tahun" maxlength="4" size="10" value="${tahunAnggaran}"  class="m-wrap medium inputnumber" readonly="true" />
                    </div>
                </div>
            </div>
            <!--<div class="form-actions fluid">
                <div class="col-md-offset-3 col-md-9">
                    <button type="button" class="btn dark" onclick='gridpagusppup()'>Cari</button>
                </div>
            </div>-->

        </form>
    </div>
</div>
<div class="portlet box">

    <div class="form-actions fluid">
        <div class="col-md-offset-3 col-md-9" align="Right">
            <!-- <button type="button" id="prosespengesahan" class="btn blue" onclick='submitvalidasi()' >Proses Pengesahan</button> -->
            <button type="button" id="prosestransfer" class="btn blue" onclick='cekstatus()' >Proses</button>

        </div>
    </div>

    <div class="portlet-title">
    </div>
    <div>
        <div class="portlet-body">
            <table id="btlspdtable" class="table table-striped table-bordered table-condensed table-hover " >
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Jenis Pajak</th>
                        <th>Nilai Pajak</th>
                        <th>Kode Billing</th>
                        <th>Status BPN</th>
                        <th>Status Transfer</th>
                        <th>Transfer</th>
                    </tr>
                </thead>
                <tbody id="btlspdtablebody" >
                </tbody>
            </table>
        </div>


    </div>

</div>


<script  type="text/javascript" src="${pageContext.request.contextPath}/static/js/aplikasi/sp2d/pajaktransfer/indextransfermulti.js"></script>
<script type="text/javascript">
                $(document).ready(function() {
                    gridmulti();
                })
</script>